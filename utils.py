        ...
        for j in xrange(N):
            ...
            if o1[0] > o2[0] and o1[1] < o2[1] and \
                ...
                for k, p in enumerate(patterns[j]):
                    if k == 0: continue # Do not add the diagonal repetition
...
def compute_segment_score_omega(X, start_i, start_j, min_dur, th, rho):
    ...
    while start_i + M < X.shape[0] and start_j + M < X.shape[0]:
        ...
        for omega in np.arange(-rho+1, rho):
            score += patch.trace(offset=omega)
        
        score -= patch.trace(offset=rho)
        score -= patch.trace(offset=-rho)
        
        den = M
        for i in np.arange(1,rho):
            den += 2*(M-i)
        score /= float(den)
    ...
    for i in xrange(N-min_dur):
        for j in xrange(i+1, N-min_dur):
            ...
            if score > th and not is_square(X, i, j, M, 0.97):
                #print score, i, j, M
                #plt.imshow(X[i:i+M, j:j+M], interpolation="nearest"); plt.show()
                X[i:i+M, j:j+M] = 0
...
def compute_CQT_filters(N, fs):
    ...
    for p in xrange(12):
        for octave in xrange(O):
            ...
            end_bin = freq2bin(min_f*2**(octave + (p+1)/12.), N , fs)
